"""Read-only epoch50 report and epoch25 preservation verification."""
import csv
import json
...
root = Path('outputs/droid_policy50_val30_20260915')
previous = Path('outputs/droid_policy25_val30_20260915')
snapshot = json.loads(Path(__file__).with_name('preserved_epoch25_sha256.json').read_text())
assert {str(p) for p in previous.rglob('*') if p.is_file()} == set(snapshot)
for path, digest in snapshot.items():
    assert sha256(path) == digest, path
print('All', len(snapshot), 'epoch25 files preserved byte-for-byte PASS')
proof = json.loads((root/'verification.json').read_text())
assert proof['complete'] and proof['fixed']['reference_epoch'] == 50
...
wanted = {(r['representation'], r['task'], int(r['hp_index']), epoch)
          for r in selected if r['representation'] in ('Policy50', 'DINO')
...
for job in json.loads((root/'run_manifest.json').read_text())['jobs']:
    ...
    assert result['signature']['reference_metadata']['reference_epoch'] == 50
print('Full canonical coverage', len(canonical), 'and macro means PASS')
print('Source/output checksums and epoch50 identity PASS; max score error', proof['canonical_max_abs_error'])
